...
export function prepareMultiTab(initialDoc, store) {
	...
	if (
		!extensions ||
		(extensions.length !== 1 && extensions.length !== initialDoc.length)
	) {
	...
	if (extensions.length === 1) {
		console.warn(
			`Extension option '${extensions[0]}' will be applied to all tabs`
		);
	}
...
export default class TabManager {
	...
	deleteTab(id) {
		const [deletedTab] = this.tabs.splice(
	...
	listenForAnnotChanges() {
		...
		core.addEventListener('annotationsLoaded', addAnnotListener, {
			once: true
		});
	...
	*[Symbol.iterator]() {
	...
	static throwError() {
		console.warn(
			'File database has encountered an error. File data will not be saved when switching tabs'
		);
	}
...
export class Tab {
	...
	async load(dispatch, db) {
		core.addEventListener(
			...
			async () => {
				...
				this.saveData.annotInDB
					? await this.restoreAnnotDataFromDBOnLoad(db)
					: this.restoreAnnotDataOnLoad();
			},
			{ once: true }
		);
	...
	async saveCurrent(db) {
		...
		if (
			this.useDB &&
			document?.type === workerTypes.PDF &&
			document.doc?.arePagesAltered
		) {
	...
	restorePageDataOnLoad() {
		...
		this.saveData.scrollTop &&
			core.addEventListener('finishedRendering', updateScroll, { once: true });
		this.saveData.zoom &&
			core.addEventListener('documentLoaded', updateZoom, { once: true });
		this.saveData.page &&
			core.addEventListener('documentLoaded', updatePage, { once: true });
	...
	async restoreAnnotDataFromDBOnLoad(db) {
		if (this.saveData.annotInDB) {
			...
			core.addEventListener('documentUnloaded', removeListeners, {
				once: true
			});
		}
	}
